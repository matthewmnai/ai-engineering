...
OUTPUT_DIR_STEP3 = os.path.join(OUTPUT_DIR, "step3_data_format")
OUTPUT_DIR_STEP4 = os.path.join(OUTPUT_DIR, "step4_finetune")
OUTPUT_DIR_STEP6 = os.path.join(OUTPUT_DIR, "step6_quick_comparison")
...
def step1_collect_and_clean(sample_size: int = 0):
    """
    ...
    输出: processed_data/ 目录下的多个JSONL文件
      ...
      - medical_raw_500.jsonl    (原数据样本及前500)
    ...
    print("\n" + "=" * 70)
    if (sample_size > 0):
        print(f"  Step 1: 数据收集与清洗, 采样 {sample_size} 条数据")
    else:
        print("  Step 1: 数据收集与清洗, 完整数据集")
    ...
    save_dataset(raw_sample, os.path.join(OUTPUT_DIR_STEP1, "medical_raw_sample.jsonl"))
    ...
    save_dataset(alpaca_data, os.path.join(OUTPUT_DIR_STEP1, "medical_alpaca_full.jsonl"))

    chat_data = to_chat_format(cleaned_data)
    save_dataset(chat_data, os.path.join(OUTPUT_DIR_STEP1, "medical_chat_full.jsonl"))

    # 1.4 均衡采样
    print("\n--- 1.4 均衡采样（快速实验用）---")
    sample_size = len(cleaned_data) if sample_size < table_size else sample_size
    sampled_data = sample_balanced_data(cleaned_data, samples_per_dept=sample_size // table_size)
    sampled_alpaca = to_alpaca_format(sampled_data)
    save_dataset(sampled_alpaca, os.path.join(OUTPUT_DIR_STEP1, "medical_alpaca_sampled.jsonl"))
    ...
    save_dataset(val_data, os.path.join(OUTPUT_DIR_STEP1, "medical_alpaca_val.jsonl"))
    save_dataset(train_data, os.path.join(OUTPUT_DIR_STEP1, "medical_alpaca_train.jsonl"))
...
def step2_quality_report():
    ...
    # 2.1 加载原始数据样本
    raw_path = os.path.join(OUTPUT_DIR_STEP1, "medical_raw_sample.jsonl")
    clean_path = os.path.join(OUTPUT_DIR_STEP1, "medical_alpaca_sampled.jsonl")
    ...
    raw_report_path = os.path.join(OUTPUT_DIR_STEP2, "raw_quality_report.json")
    ...
    clean_report_path = os.path.join(OUTPUT_DIR_STEP2, "clean_quality_report.json")
...
def step3_format_conversion():
    ...
    input_path = os.path.join(OUTPUT_DIR_STEP1, "medical_alpaca_train.jsonl")
    ...
    save_as_jsonl(chat_data, os.path.join(OUTPUT_DIR_STEP3, "medical_chat_converted.jsonl"))
    ...
    save_as_jsonl(sft_data, os.path.join(OUTPUT_DIR_STEP3, "medical_unsloth_sft.jsonl"))
...
def step4_finetune():
    ...
    train_data_path = os.path.join(OUTPUT_DIR_STEP1, "medical_alpaca_train.jsonl")
...
def step5_evaluation():
    ...
    # 尝试加载Step 4的真实推理结果
    eval_path = os.path.join(OUTPUT_DIR_STEP4, "processed_data", "eval_results.json")
...
def step6_cleaning_value():
    ...
    comparison = comparison_module.run_comparison(
        use_gpu=use_gpu, data_dir=DATA_DIR, model_name=MODEL_NAME, output_dir=OUTPUT_DIR_STEP6)
...
def run_all(sample_size: int = 0):
    ...
    step1_collect_and_clean(sample_size)
...
def main():
    ...
    parser.add_argument("--sample_size", type=int, default=-1,
                       help="指定采样数据量（0 表示使用完成数据集）")
    args = parser.parse_args()

    if args.step in (None, 1):
        if args.sample_size < 0:
            raise ValueError("【强制参数】执行完整流程 或 step=1 时，必须通过 --sample_size 指定大于0的采样数量！")

    step_funcs = {
        1: lambda: step1_collect_and_clean(args.sample_size),
        2: lambda: step2_quality_report(),
        3: lambda: step3_format_conversion(),
        4: lambda: step4_finetune(),
        5: lambda: step5_evaluation(),
        6: lambda: step6_cleaning_value(),
    }
